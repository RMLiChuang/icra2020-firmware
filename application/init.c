...
{
  glb_sys_cfg = HAL_GPIO_ReadPin(SYS_CFG_Port, SYS_CFG_Pin);//PD6
}
...
void hw_init(void)
{
  cali_param_init();//读取云台校准数据
  board_config();//板载外设初始化，相关函数进行注册
  test_init();
  system_config();//检测串口2 R引脚电平，低电平为底盘模式，否则为云台模式
  ulog_init();
  ulog_console_backend_init();
  
  referee_param_init();
  usart3_rx_callback_register(referee_uart_rx_data_handle);//裁判系统数据接收
  referee_send_data_register(usart3_transmit);//给裁判系统发送数据

  if(glb_sys_cfg == CHASSIS_APP)
  {
    rc_device_register(&rc_dev, "uart_rc", 0);//接收数据函数注册  底盘模式下接收数据为串口
  ...
  {
    rc_device_register(&rc_dev, "can_rc", 0);//接收数据函数注册  云台模式下接收数据为can
