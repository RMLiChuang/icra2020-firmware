...
void infantry_cmd_task(void const *argument)
{
  ...
  if (app == CHASSIS_APP)
  {
    ...
    protocol_rcv_cmd_register(CMD_PUSH_GIMBAL_INFO, gimbal_info_rcv);		//云台数据
  ...
  while (1)
  {
    if (rc_device_get_state(prc_dev, RC_S2_DOWN) != RM_OK)//如果S2未处于下方
    {
...
int32_t gimbal_push_info(void *argc)
{
  ...
  if (get_gimbal_init_state() == 1) //在云台校准过程中，失能底盘跟随云台功能，防止云台转动时，底盘跟着转动
  {
    cmd_gimbal_info.yaw_ecd_angle = 0;
  }

  protocol_send(PROTOCOL_BROADCAST_ADDR, CMD_PUSH_GIMBAL_INFO, &cmd_gimbal_info, sizeof(cmd_gimbal_info));//将云台数据广播出去

  return 0;
}

int32_t chassis_push_info(void *argc)//将底盘数据发布给妙算
{
