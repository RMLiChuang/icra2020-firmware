...
int32_t chassis_gyro_updata(struct chassis *chassis, float yaw_angle, float yaw_rate)//底盘角度与角速度更新(来源于底盘陀螺仪数据)
{
  if (chassis == NULL)
    return -RM_INVAL;
...
int32_t chassis_get_info(struct chassis *chassis, struct chassis_info *info)
{
  ...
  info->yaw_gyro_rate = chassis->mecanum.gyro.yaw_gyro_rate; //底盘角速度
